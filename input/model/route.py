...
from input.model.nodes import node, end_system
...
class route:
    ...
    def init_from_node_mapping(self, node_mapping: Set[Tuple[str, str]]):
        """
    ...
    def get_all_links_dfs(self, tc) -> List[link]:
        return [tc.L_from_nodes[tpl[0]][tpl[1]] for tpl in nx.dfs_edges(self._DAG, source=self.stream.sender_es_id)]
    ...
    def get_all_es_and_links_dfs(self, tc) -> List[Union[node, link]]:
        ...
        """
        l = []
        for tpl in nx.dfs_edges(self._DAG, source=self.stream.sender_es_id):
            if tpl[0] is self.stream.sender_es_id:
                l.append(tc.ES[tpl[0]])
            l.append(tc.L_from_nodes[tpl[0]][tpl[1]])
            if tpl[1] in self.stream.receiver_es_ids:
    ...
    def get_predeccessor_links(self, l_or_es_id: str, tc) -> Optional[List[link]]:
        """
        :return: List of predecessor links
        """
        if l_or_es_id in tc.L:
            l = tc.L[l_or_es_id]
            return self.get_predeccessor_links_for_node(l.src.id, tc)
        elif l_or_es_id in tc.N:
            return self.get_predeccessor_links_for_node(l_or_es_id, tc)
        else:
            return []

    def get_predeccessor_links_and_es(self, l_or_es_id: str, tc) -> List[Union[link, end_system]]:
        """
