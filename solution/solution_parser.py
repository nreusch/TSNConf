...
def get_solution_general_info_dataframe(solution: Solution) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        columns, columns=["Testcase Name", "# ES", "# Switches", "# Streams"]
    )
...
def get_solution_results_info_dataframe(solution: Solution) -> pd.DataFrame:
    ...
    row.append("{:.0f}".format(solution.cost))
    if len(solution.stream_costs.values()) != 0:
        row.append("{:.0f}".format(sum(solution.stream_costs.values())/len(solution.stream_costs.values())))
    else:
        row.append("0")
    row.append("{:.0f}".format(solution.timing_object.time_first_feasible_solution))
    row.append("{}/{}".format(solution.infeasible_streams, len(solution.tc.F)))
